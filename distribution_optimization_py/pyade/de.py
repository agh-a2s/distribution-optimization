...
def apply(
    ...
    max_evals: int,
    seed: Union[int, None],
    init_population: Callable = default_init_population,
    ...
    if (type(f) is not int and type(f) is not float) or not 0 <= f <= 2:
        raise ValueError("f (mutation parameter) must be a " "real number in [0,2].")

    if (type(cr) is not int and type(cr) is not float) or not 0 <= cr <= 1:
        raise ValueError("cr (crossover ratio) must be a " "real number in [0,1].")
    ...
    if type(bounds) is not np.ndarray or bounds.shape != (individual_size, 2):
        raise ValueError(
            "bounds must be a NumPy ndarray.\n"
    ...
    for current_generation in range(max_iters):
        mutated = binary_mutation(population, f, bounds)
        if cross == "bin":
